...
#if 1 // Syscard additions
#define VS_DRPB_TESTER_PACKET_TX_RX_COMMAND_OPCODE               ((Word_t)(0xFD85))
#define DRPB_TESTER_PACKET_TX_RX_SIZE                            12
#define VS_ACL_PACKET_TYPE_MAX                                   0x0b
#define VS_ACL_PACKET_PATTERN_MAX                                0x05
#define VS_ACL_PACKET_LEN_MAX                                    1021
...
int BTPSAPI VS_Set_Max_Output_Power(unsigned int BluetoothStackID, Byte_t MaxPower)
{
   ...
   if((BluetoothStackID) && (MaxPower >= 0) && (MaxPower <= 12))
   {
      ...
      if(!ret_val)
      {
         ...
         if((ret_val = MapSendRawResults(ret_val, Status, Length, ReturnBuffer)) == 0)
#if 1 // Syscard as indicated by TI engineer on E2E forum
         {
             ...
             ASSIGN_HOST_DWORD_TO_LITTLE_ENDIAN_UNALIGNED_DWORD(&CommandBuffer[1],0x00000800);
...
int BTPSAPI VS_Enable_FCC_Test_Mode(unsigned int BluetoothStackID, VS_Modulation_Type_t Modulation_Type, Byte_t Test_Pattern, Byte_t Frequency_Channel, Byte_t Power_Level, DWord_t Generator_Init_Value, DWord_t EDR_Generator_Mask)
{
   ...
   /* Before continuing, make sure the input parameters appears to be   */
   /* semi-valid.                                                       */
   if((BluetoothStackID) && (Modulation_Type >= mtContinuousWave) && (Modulation_Type <= mtBLE) && (Test_Pattern >= VS_TEST_PATTERN_PN9) && (Test_Pattern <= VS_TEST_PATTERN_USER_DEFINED) && (Frequency_Channel <= VS_MAXIMUM_BT_FREQUENCY_CHANNEL) && (Power_Level <= VS_MAXIMUM_POWER_LEVEL))
   {
...
#if 1 // Syscard additions
   /* The following function is used to starts sending/receiving packets */
   /* using packet transmission parameters such as frequency channel, */
   /* packet type, and packet length. It is used for Packet TX/RX */
   /* This function returns zero if successful or a negative return     */
   /* error code if there was an error.                                 */
int BTPSAPI VS_DRPb_Tester_Packet_Tx_Rx(unsigned int BluetoothStackID, Byte_t Frequency_Mode, 
                                        Byte_t Tx_Single_Frequency, Byte_t Rx_Single_Frequency, 
                                        Byte_t ACL_Packet_Type, Byte_t ACL_Packet_Pattern, 
                                        Word_t ACL_Packet_Len, Byte_t Power_Level, 
                                        Byte_t Dis_Whitening, Word_t PRBS9_Val)
{
   int    ret_val;
   Byte_t Length;
   Byte_t Status;
   Byte_t OGF;
   Word_t OCF;
   Byte_t CommandBuffer[DRPB_TESTER_PACKET_TX_RX_SIZE];
   Byte_t ReturnBuffer[1];

   /* Before continuing, make sure the input parameters appears to be   */
   /* semi-valid.                                                       */
   if((BluetoothStackID) && (Frequency_Mode <= 3) && (Tx_Single_Frequency <= VS_MAXIMUM_BT_FREQUENCY_CHANNEL) && 
      ((Rx_Single_Frequency <= VS_MAXIMUM_BT_FREQUENCY_CHANNEL) || (Rx_Single_Frequency == 0xff)) && 
      (ACL_Packet_Type <= VS_ACL_PACKET_TYPE_MAX) && (ACL_Packet_Pattern <= VS_ACL_PACKET_PATTERN_MAX) &&
      (ACL_Packet_Len <= VS_ACL_PACKET_LEN_MAX) && (Power_Level <= VS_MAXIMUM_POWER_LEVEL) &&
      (Dis_Whitening <= 1) && (PRBS9_Val <= 0x01ff))
   {
      /* Format the HCI VS DRPB Tester Packet Tx Rx Command.                  */
      BTPS_MemInitialize(CommandBuffer, 0, DRPB_TESTER_PACKET_TX_RX_SIZE);

      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[0], Frequency_Mode);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[1], Tx_Single_Frequency);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[2], Rx_Single_Frequency);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[3], ACL_Packet_Type);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[4], ACL_Packet_Pattern);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[5], 0);
      ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&CommandBuffer[6], ACL_Packet_Len);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[8], Power_Level);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[9], Dis_Whitening);
      ASSIGN_HOST_WORD_TO_LITTLE_ENDIAN_UNALIGNED_WORD(&CommandBuffer[10], PRBS9_Val);

      Length  = sizeof(ReturnBuffer);
      OGF     = VS_COMMAND_OGF(VS_DRPB_TESTER_PACKET_TX_RX_COMMAND_OPCODE);
      OCF     = VS_COMMAND_OCF(VS_DRPB_TESTER_PACKET_TX_RX_COMMAND_OPCODE);
      ret_val = HCI_Send_Raw_Command(BluetoothStackID, OGF, OCF, DRPB_TESTER_PACKET_TX_RX_SIZE, (Byte_t *)CommandBuffer, &Status, &Length, ReturnBuffer, TRUE);
   ...
   /* The following function is used to starts continuous reception at the */
   /* selected frequency */
int BTPSAPI VS_DRPb_Tester_Con_Rx(unsigned int BluetoothStackID, Byte_t Frequency) 
{
   int    ret_val;
   Byte_t Length;
   Byte_t Status;
   Byte_t OGF;
   Word_t OCF;
   Byte_t CommandBuffer[DRPB_TESTER_CON_RX_SIZE];
   Byte_t ReturnBuffer[1];

   /* Before continuing, make sure the input parameters appears to be   */
   /* semi-valid.                                                       */
   if((BluetoothStackID) && (Frequency <= VS_MAXIMUM_BT_FREQUENCY_CHANNEL))
   {
      /* Format the HCI VS DRPB Tester Packet Tx Rx Command.                  */
      BTPS_MemInitialize(CommandBuffer, 0, DRPB_TESTER_CON_RX_SIZE);

      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[0], Frequency);
      ASSIGN_HOST_BYTE_TO_LITTLE_ENDIAN_UNALIGNED_BYTE(&CommandBuffer[1], 0);
